...
- `main2main_flow/scripts/utils/` — deterministic helpers and shared utilities:
  ...
  - `ci_config_guard.py` — CI-config write guard (run 35513059821: the adapter "fixed" failing UT by editing a nightly workflow yaml; GitHub rejected the push 6/6 on PAT workflow scope). Whitelist policy (2026-09-21): under `.github/` only `vllm-main-verified.commit` and `vllm-release-tag.commit` may change — everything else (workflows, composite actions, CODEOWNERS, templates) is guarded, deliberately broader than GitHub's own workflow-scope check. Three mechanical layers: brackets every opencode adapter session with a snapshot/restore so a CI-config cheat can't be verified green, strips the dir at both squash chokepoints, and the push preflight aborts loudly on a committed one
