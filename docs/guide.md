...
| `MAIN2MAIN_EXTENDED_E2E` | 设为 `0` 把 gate 主 e2e 退回旧版 23 例回归集（默认 `1` 跑 36 例扩展集 + own-diff 分诊，见 Step 3c+） | `1` |
| `MAIN2MAIN_EXTENDED_MODE` | `fixed`（默认）跑 `test_policy.json` `extended_e2e` 固定集（36 例、失败面配权、无 310p，~17min）；`full` 解析 label 全集减去各类扣除（数小时） | `fixed` |
| `MAIN2MAIN_EXTENDED_FIX_ROUNDS` | gate 主 e2e own-diff 失败后的 adapter-fix 轮数（每轮只重跑失败 suite） | `2` |
| `MAIN2MAIN_EXTENDED_MAX_MIN` | gate 主 e2e 墙钟兜底（分钟），`0` 关闭 | `360` |
| `MAIN2MAIN_EXTENDED_TEST_CASES` | 空白分隔的 gate 主 e2e 用例列表，整体替代两种来源（漂移护栏仍生效） | — |
...
当前 allowlist 固定 23 个用例（one_card 13 + two_card 5 + four_card 5，含节点级条目），blocklist 7 项（不稳定的超大模型用例与 one_card 的 dspark spec decoding 节点——dspark 由 four_card 的 w4a8 节点代表）。用例集由 `tests/test_policy_duration_budget.py` 钉住（makespan pin）：每个用例必须带上游实测时长快照（`_RECORDED_S`，取自 vllm-ascend `test_config.yaml` 的 `estimated_times`），且整个集合满足硬约束——贪心调度不超过 **4 轮**、任一单用例时长不超过 **20 分钟**（1200s）、全部用例总 wall 不超过 **20 分钟**（1200s；2026-09-15 二次修订放宽自 15min，当日首次修订曾从 25min 收紧到 15min，实测过紧）。2026-09-15 首轮把 7 个重用例降级进 extended_e2e，同日二次修订回纳 5 个（disaggregated_encoder、prefix_caching、deepseek_v3_2_w8a8_pruning、vlm、test_basic graph_mode 节点——1200s 预算下穷举的最大子集），仅 pipeline_parallel（524s，无近期失败史，PP 拓扑由上游 PR CI 覆盖）与 qwen3_mrv2_eplb（300s，eplb 面在 extended 有两层覆盖）留 extended。记录墙钟 1164s（19.4min、4 轮），dspark V2 节点（401s）留守作 V2 哨兵。改选集必须同步更新 `_RECORDED_S` 快照并复算 makespan（CI 自动校验）。
...
### Step 3c+ — gate 主 e2e `_run_gate_e2e`（扩展覆盖集 + own-diff 分诊）

在 gate 静态绿之后、release smoke 之前执行。2026-09-15 起这就是 gate 的回归 e2e——**原来的 gate 后扩展阶段已并入，不再单独设步骤**（用户拍板：gate 的 main e2e 换成扩展集范围）。触发时机保持原状：最后一步跳过/失败了 per-step e2e，或 gate 修复改动了树；静态与 smoke 一样按树 sha 记忆化（绿树/已分诊继承的树不重跑）。背景：每步固定集只能证明它包含的用例——2026-09-15 的 run 在 pre_ci 全绿下送出 PR 16575，上游 CI 却在固定集从未触及的腿上失败（dflash/dspark release lane、PCP spec decode）。

**用例源**（`extended_e2e.py`，默认 `MAIN2MAIN_EXTENDED_MODE=fixed`）：`test_policy.json` 的 **`extended_e2e` 固定键**——36 例（2026-09-16 四次修订，55 例裁剪：24 单卡 + 7 双卡 + 5 四卡），**按最近 CI 失败面配权**（依据 vllm-report `pr_ci_results/2026-09-15-failure-analysis.json`，窗口 09-08→09-15 共 15 类）：extract_hidden_states（3 个 PR 复现，含 release lane MRV2 不支持类）、gumbel_sample、mamba copy-funcs、Kimi-K3 DSpark drafter、DSparkSpeculator draft_watermarker、PCP NPU-op 崩溃族（per-step 已有节点）各收其对应 e2e 文件，六个失败加权锚点全保留；2/4 卡覆盖 12 条，**结构性排除全部 `_310p` 套件**（a3-16 池绝对跑不了 310P 硬件路径，两种模式下运行时都剔除，override 也不例外）。裁掉的 19 例是高耗时且无近期失败史的面（两条普通 data_parallel、gemma4、qwen3moe_lora、双卡 qwen3_moe_eplb、llama32/reranker lora、draft_parallel/ngram/suffix、batch_invariant_tp4、profiling_chunk、minicpm、minimax sparse_attn 单卡、xlite、multistream_overlap、npu_ipc_weight_transfer、graphex_norm_quant_fusion、completion_with_prompt_embeds）——被裁面仍有代表：DP 由 spec_decode_data_parallel 守、EPLB 由四卡 mrv2_eplb + 30s eplb_map 守、minimax 由四卡 m3 守。pipeline_parallel（524s、无近期失败史）此前已被裁出——PP 拓扑由上游 PR CI 覆盖。预测墙钟 ~17min（与 55 例版 ~31min 同一 4/2/2 槽位并行模型：单卡 lane est 69min/4、双卡 24min/2、四卡 28.7min/2 取 max；零样本 est×0.69 校准——文件级 est/实测 中位比 1.44；首轮实跑的 elapsed_s 是校准点，偏了再裁）。离线筛选用例，运行时有四个护栏兜住列表过期：allowlist 已覆盖（策略漂移，那些每步都跑）、上游 `skip_tests` 新收入、`_310p` 套件、树上已消失（2026-09-13 幻影文件教训），命中即剔除并记录。`MAIN2MAIN_EXTENDED_MODE=full` 切换为全集解析器（数小时），`MAIN2MAIN_EXTENDED_TEST_CASES` 整体覆盖两种来源（护栏仍生效）。排序（两种模式相同）：测试文件 import 的 `vllm_ascend.*`/`tests.*` 模块与适配 diff 相交的为 tier1 先跑，其余随后，两层内部按预估时长升序（`run_tests` 以 `preserve_order=True` 调用，调度器不再重排）。注意 `main2main_tests.json` 不是来源：它是定时回归子集（18 条）且全部 ⊆ 固定集。

**失败分诊**（release smoke 同款哲学，应用于全集）：首轮失败后**只重跑失败的 suite**（delta，同树一次，吸收 flake，不动 adapter）；仍失败则按 traceback 文件 ∩ 累计 adaptation diff 分诊——交集为空 → **upstream-inherited**（#16382 形态），写 `quality_gate/gate_e2e_inherited.json` 留证、**不阻塞**、不烧 adapter；交集非空或无法提取文件（保守 own）→ **own-diff**，进 adapter-fix 轮（`MAIN2MAIN_EXTENDED_FIX_ROUNDS`，默认 2），每轮只重跑失败 suite，预算含墙钟兜底（`MAIN2MAIN_EXTENDED_MAX_MIN`，默认 360）+ 失败集合与上轮相同即 stop-loss。pytest exit 4（collection error）的 suite 永久移出修复轮（adapter 修不了 import 层损坏），当只剩它们时分诊结论决定是否阻塞。修复使树 sha 变化时先重跑 gate 静态（静态失败 → 1 轮 adapter → 仍败则以 static_failed 结束、保留修复）。

**与旧扩展阶段的关键差异**：本 e2e 在 gate **内部**——own-diff 预算耗尽即 **gate 失败、不 push**（上游 CI 裁决的是继承风险，不是已证明的 own 回归）；继承风险照常 push 留给上游 PR CI + pr_ci_watch。修复不单独 commit（与 gate 其他修复一致留在工作树），gate 成功路径统一重新生成 `gate_final_patch`。证据：`quality_gate/gate_e2e_result.json`（status/mode/rounds/fix_actions/statics/budgets/dropped_*/final/inherited_files）+ `final_summary.md` 与 PR body 的 "Gate e2e" 段。`MAIN2MAIN_EXTENDED_E2E=0` 退回**旧版 23 例回归 e2e**（`_run_e2e_test_for_final_gate`：同树 flake 重试 + 确定性失败 revert）。
...
├── quality_gate/                         # final quality gate 产物（final_gate.patch、gate_e2e_result.json、gate_e2e_inherited.json、gate-e2e/tests/ 轮日志、gate-e2e-statics/）
└── steps/
    ├── step-1/
    │   ├── upstream.patch
    │   ├── changed_files.txt
