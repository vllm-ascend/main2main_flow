"""Final quality gate: format + mypy check before push.
...
"Run mypy" + "Run pre-commit" steps - by running them here on the final
cumulative diff, we catch format/mypy issues in the exact environment CI
will use, and give the adapter a chance to fix them before push.
...
from main2main_flow.scripts.utils.pre_ci_check import _check_format, _check_mypy
...
def run_final_quality_gate(
    ...
    ts_print("[final_quality_gate] running format + mypy on final diff...")
    ...
    if all_passed:
        ts_print("[final_quality_gate] PASSED (format + mypy clean)")
        return True, []
    ts_print(f"[final_quality_gate] FAILED -> {gate_path}")
    for c in checks:
        if not c["passed"]:
